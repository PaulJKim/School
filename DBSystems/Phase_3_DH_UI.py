...
class Phase3:
    ...
    ## initialize all windows ##
        ...
        self.category_OPTIONS = ['Computing for good', 'Doing good for your neighborhood', \
                             'Reciprocal teaching and learning', 'Urban development', \
                             'Adaptive learning', 'Technology for social good', 'Sustainable communities', \
...
## ADMIN PAGES ##
    ...
    def addProj(self):
        ...
        estNumStu_text = StringVar()
        estNumStu = Entry(self.addProjWin, textvariable=estNumStu_text, width=30)
        estNumStu.grid(row=8, column=2)
        ...
        self.majorReq_OPTIONS = ['Only CS Students', 'Only IE students']
        self.yearReq_OPTIONS = ['Only Senior Students', 'Only Junior Students']
        self.deptReq_OPTIONS = ['No Requirement', 'idk']
        ...
        self.majorReq_adminProj.set(self.major_OPTIONS[0])
        self.yearReq_adminProj.set(self.year_OPTIONS[0])
        ...
        yearReq_optionMenu = OptionMenu(self.addProjWin, self.yearReq_adminProj, *self.year_OPTIONS, command=self.yearReqOptionChanged_proj)
    ...
    def addCourse(self):
        ...
        desigLab.grid(row=5, column=1)
        categLab.grid(row=6, column=1)
        estStuLab.grid(row=7, column=1)

        #entry boxes
        courseNumEntry_text = StringVar()
        courseNameEntry_text = StringVar()
        instructEntry_text = StringVar()
        estStuEntry_text = StringVar()

        courseNumEntry = Entry(self.addCourseWin, textvariable=courseNumEntry_text, width=25)
        courseNameEntry = Entry(self.addCourseWin, textvariable=courseNameEntry_text, width=25)
        instructEntry = Entry(self.addCourseWin, textvariable=instructEntry_text, width=25)
        estStuEntry = Entry(self.addCourseWin, textvariable=estStuEntry_text, width=25)

        courseNumEntry.grid(row=2, column=2)
        courseNameEntry.    grid(row=3, column=2)
        instructEntry.grid(row=4, column=2)
        estStuEntry.grid(row=7, column=2)
    ...
    def checkUserLogin(self):
        ...
        if userName == testUsername and passWord == testPassword:
            ...
            messagebox.showerror("Incorrect Login Information", "The Username and Password do not match, please try again "
    ...
    def desigOptionChanged_proj(self, passedStr):
        desigOption = self.desigText_adminCourse.get()
        print('passed string:', passedStr)
